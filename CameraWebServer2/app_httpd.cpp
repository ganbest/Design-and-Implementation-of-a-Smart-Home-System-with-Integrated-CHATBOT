...
// Hàm TFT khai báo trong CameraWebServer2.ino
extern void displayFace(const uint8_t* buf, size_t len, const char* name);
extern void displayAlert();
extern void displayTest();
extern void displayLiveFrame(const uint8_t* buf, size_t len);

// Handler: /open?name=TAN
static esp_err_t open_handler(httpd_req_t *req) {
  ...
  if (fb && fb->format == PIXFORMAT_JPEG) {
    displayFace(fb->buf, fb->len, name);
  }
...
static esp_err_t stream_handler(httpd_req_t *req) {
  ...
  while (true) {
    ...
    // Hien thi frame nay len TFT (cung luc voi web stream)
    displayLiveFrame(_jpg_buf, _jpg_buf_len);
    if (fb) {
      esp_camera_fb_return(fb);
      fb = NULL;
      _jpg_buf = NULL;
